...
class CameraType {
 public:
  ...
  sensor_msgs::msg::Image::ConstSharedPtr get_image_msg() { return image_msg_; }
  ...
  void set_camera_utils(
    ...
    camera_matrix_cv_ =
        (cv::Mat_<double>(3, 3) << msg->k[0], 0.000000, msg->k[2], 0.000000,
         msg->k[4], msg->k[5], 0.000000, 0.000000, 1.000000);
    ...
    distortion_matrix_cv_ = (cv::Mat_<double>(1, 5) << msg->d[0], msg->d[1],
                             msg->d[3], msg->d[4], msg->d[2]);

    is_info_initialized_ = true;
  }

  void set_lidar_to_camera_matrix(geometry_msgs::msg::TransformStamped &msg) {
    std::cout<<"transform: "<<msg.transform.translation.x<<" "
             <<msg.transform.translation.y<<" "
             <<msg.transform.translation.z<<std::endl;
    std::cout<<"rotation: "<<msg.transform.rotation.x<<" "
    ...
    //           << affine_lidar_to_camera_matrix.matrix() << std::endl;
  ...
  void set_lidar_to_camera_projection_matrix() {
    // lidar_to_camera_projection_matrix_ = projection_matrix_ *
    // lidar_to_camera_matrix_.block<4, 4>(0, 0);
    Eigen::Matrix4d lidar_to_camera_matrix_inv =
        lidar_to_camera_matrix_.inverse();
    lidar_to_camera_projection_matrix_ =
        projection_matrix_ * lidar_to_camera_matrix_inv.block<4, 4>(0, 0);
  }
