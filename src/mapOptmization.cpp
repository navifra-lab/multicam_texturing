...
class mapOptimization : public ParamServer {
 public:
  ...
  //    pcl::PointCloud<pcl::PointXYZRGB>::Ptr laserCloudCornerLastRGB; //
  //    downsampled rgb corner feature pcl::PointCloud<pcl::PointXYZRGB>::Ptr
  //    laserCloudSurfLastRGB; // downsampled rgb surf feature
  ...
  void camera_callback() {
    std::for_each(
        ...
        [this](const std::pair<std::string, color_point_cloud::CameraTypePtr>
                   &pair) {
          ...
          if (!pair.second->is_info_initialized()) {
            RCLCPP_INFO(this->get_logger(), "Camera info is setting: %s",
                        pair.first.c_str());
          ...
          if (!pair.second->is_transform_initialized() &&
            ...
            // RCLCPP_INFO(this->get_logger(), "Camera transform is setting:
            // %s",
            //             pair.second->get_camera_frame_id().c_str());
            std::optional<geometry_msgs::msg::TransformStamped> transform =
                (*transform_provider_ptr_)(pair.second->get_camera_frame_id(),
                                           lidarFrame);

            // (*transform_provider_ptr_)(lidarFrame,
            //                            pair.second->get_camera_frame_id());
  ...
  mapOptimization(const rclcpp::NodeOptions &options)
    ...
    auto saveMapService =
      ...
      // int unused = system((std::string("exec rm -r ") +
      // saveMapDirectory).c_str()); unused = system((std::string("mkdir -p ") +
      // saveMapDirectory).c_str()); save key frame transformations
      ...
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalCornerCloud(
          new pcl::PointCloud<pcl::PointXYZRGB>());
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalCornerCloudDS(
          new pcl::PointCloud<pcl::PointXYZRGB>());
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalSurfCloud(
          new pcl::PointCloud<pcl::PointXYZRGB>());
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalSurfCloudDS(
          new pcl::PointCloud<pcl::PointXYZRGB>());
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalMapCloud(
          new pcl::PointCloud<pcl::PointXYZRGB>());
      ...
      if (req->resolution != 0) {
        ...
        downSizeFilterCornerRGB.setLeafSize(req->resolution, req->resolution,
                                            req->resolution);
        ...
        downSizeFilterSurfRGB.setLeafSize(req->resolution, req->resolution,
                                          req->resolution);
    ...
    srvSaveMap = create_service<lio_sam::srv::SaveMap>("lio_sam/save_map",
                                                       saveMapService);
    ...
    // Color Map
    downSizeFilterCornerRGB.setLeafSize(
        mappingCornerLeafSize, mappingCornerLeafSize, mappingCornerLeafSize);
    downSizeFilterSurfRGB.setLeafSize(mappingSurfLeafSize, mappingSurfLeafSize,
                                      mappingSurfLeafSize);
    ...
    for (const auto &camera_topic : cameraTopics) {
      ...
      color_point_cloud::CameraTypePtr camera_type_ptr =
          std::make_shared<color_point_cloud::CameraType>(image_topic,
                                                          camera_info_topic);
      camera_type_stdmap_[camera_topic] = camera_type_ptr;

      this->image_subscribers_.push_back(
          this->create_subscription<sensor_msgs::msg::Image>(
              image_topic, rclcpp::SensorDataQoS(),
              [this, image_topic, camera_topic](
                  const sensor_msgs::msg::Image::ConstSharedPtr &msg) {
                //                         RCLCPP_INFO(this->get_logger(),
                //                         "Received image on topic %s",
                //                         camera_topic.c_str());
                camera_type_stdmap_[camera_topic]->set_image_msg(msg);
              }));

      this->camera_info_subscribers_.push_back(
          this->create_subscription<sensor_msgs::msg::CameraInfo>(
              camera_info_topic, rclcpp::SensorDataQoS(),
              [this, camera_info_topic, camera_topic](
                  const sensor_msgs::msg::CameraInfo::ConstSharedPtr &msg) {
                //                         RCLCPP_INFO(this->get_logger(),
                //                         "Received camera_info on topic %s",
                //                         camera_topic.c_str());
                camera_type_stdmap_[camera_topic]->set_camera_info(msg);
                // if (camera_type_stdmap_[camera_topic]->get_image_msg() ==
                // nullptr ||
                // camera_type_stdmap_[camera_topic]->get_camera_info() ==
                // nullptr) {
                //       return;
                //   }
                //   if
                //   (!camera_type_stdmap_[camera_topic]->is_info_initialized())
                //   {
                //       RCLCPP_INFO(this->get_logger(), "Camera info is
                //       setting: %s", camera_topic.c_str());
                //       camera_type_stdmap_[camera_topic]->set_camera_utils(camera_type_stdmap_[camera_topic]->get_camera_info());
                //   }
                //   if
                //   (!camera_type_stdmap_[camera_topic]->is_transform_initialized()
                //   &&
                //   camera_type_stdmap_[camera_topic]->is_info_initialized()) {
                //       std::optional<geometry_msgs::msg::TransformStamped>
                //       transform = (*transform_provider_ptr_)(
                //               camera_type_stdmap_[camera_topic]->get_camera_frame_id(),
                //               lidarFrame);
                //       if (!transform.has_value()) {
                //           return;
                //       }
    ...
    transform_provider_ptr_ =
        std::make_shared<color_point_cloud::TransformProvider>(
            this->get_clock());
    // TODO duration
    const auto period_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::duration<double>(0.5));
    camera_timer_ = rclcpp::create_timer(
  ...
  ~mapOptimization() {
    ...
    // int unused = system((std::string("exec rm -r ") +
    // savePCDDirectory).c_str()); unused = system((std::string("mkdir ") +
    // savePCDDirectory).c_str());
    // auto save_key_pose3d = *cloudKeyPoses3D;
    // auto save_key_pose6d = *cloudKeyPoses3D;
    // pcl::io::savePCDFileBinary(savePCDDirectory + "trajectory.pcd",
    //                            *cloudKeyPoses3D);
    // pcl::io::savePCDFileBinary(savePCDDirectory + "transformations.pcd",
    //                            *cloudKeyPoses6D);
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalCornerCloud(
        new pcl::PointCloud<pcl::PointXYZRGB>());
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalCornerCloudDS(
    ...
    downSizeFilterCornerRGB.setInputCloud(globalCornerCloud);
    downSizeFilterCornerRGB.filter(*globalCornerCloudDS);
    // pcl::io::savePCDFileBinary(savePCDDirectory + "cloudCorner.pcd",
    //                            *globalCornerCloudDS);
    downSizeFilterSurfRGB.setInputCloud(globalSurfCloud);
    downSizeFilterSurfRGB.filter(*globalSurfCloudDS);
    // pcl::io::savePCDFileBinary(savePCDDirectory + "cloudSurf.pcd",
    //                            *globalSurfCloudDS);
    *globalMapCloud += *globalCornerCloud;
    *globalMapCloud += *globalSurfCloud;

    downSizeFilterSurfRGB.filter(*globalMapCloud);
    pcl::io::savePCDFileBinary(savePCDDirectory + "cloudGlobal.pcd",
                               *globalMapCloud);
    // cout << "****************************************************" << endl;
    // cout << "Saving map to pcd files completed" << endl;
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr filteredMap(new
    // pcl::PointCloud<pcl::PointXYZRGB>()); for (const auto &pt :
    // globalMapCloud->points)
    // {
    ...
    // // [2] 저장
    // pcl::io::savePCDFileASCII(savePCDDirectory + "cloudGlobal_2.pcd",
    // *filteredMap);
  }
  ...
  void allocateMemory() {
    ...
    laserCloudCornerLast.reset(
        new pcl::PointCloud<pcl::PointXYZRGB>());  // corner feature set from
                                                   // odoOptimization
    laserCloudSurfLast.reset(
        new pcl::PointCloud<pcl::PointXYZRGB>());  // surf feature set from
                                                   // odoOptimization
    laserCloudCornerLastDS.reset(
        new pcl::PointCloud<pcl::PointXYZRGB>());  // downsampled corner featuer
                                                   // set from odoOptimization
    laserCloudSurfLastDS.reset(
        new pcl::PointCloud<pcl::PointXYZRGB>());  // downsampled surf featuer
                                                   // set from odoOptimization
  ...
  void laserCloudInfoHandler(const lio_sam::msg::CloudInfo::SharedPtr msgIn) {
    ...
    std::for_each(
        camera_type_stdmap_.begin(), camera_type_stdmap_.end(),
        [this, msgIn](const std::pair<std::string,
                                      color_point_cloud::CameraTypePtr> &pair) {
          if (pair.second->get_image_msg() == nullptr ||
              pair.second->get_camera_info() == nullptr ||
              !pair.second->is_info_initialized() ||
          ...
          //   rclcpp::Time
          //   image_time(pair.second->get_image_msg()->header.stamp); double
          //   time_diff = std::fabs((lidar_time - image_time).seconds()); if
          //   (time_diff > 0.2)
          //   {
          ...
          //       //   ++time_diff_count;
          //       //   double avg_time_diff = total_time_diff /
          //       time_diff_count;
          //       //   RCLCPP_INFO(this->get_logger(),
          //       //               "[Time Diff] Current: %.6f s | Avg: %.6f s |
          //       Max: %.6f s | Min: %.6f s",
          //       //               time_diff, avg_time_diff, max_time_diff,
          //       min_time_diff);
          //   }
          ...
          if (image.type() != CV_8UC3) {
            RCLCPP_WARN(this->get_logger(),
                        "Image type is not CV_8UC3. Got: %d", image.type());
            return;
          }
...
#pragma omp parallel for num_threads(numberOfCores)
          for (size_t i = 0; i < cloud_corner.getPointCount(); ++i) {
            ...
            //   if (point3d_transformed_camera[2] > 0 && x >= 0.5 && x <
            //   pair.second->get_image_width() - 0.5 && y >= 0.5 && y <
            //   pair.second->get_image_height() - 0.5)
            //   {
            //       cv::Mat patch;
            //       cv::getRectSubPix(image, cv::Size(1, 1), cv::Point2f(x, y),
            //       patch); if (!patch.empty())
            //       {
            //           pointRGB.x = point.x;
            //           pointRGB.y = point.y;
            //           pointRGB.z = point.z;
          ...
          color_point_cloud::PointCloudConst cloud_surface{
              msgIn->cloud_surface};
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    if (laserCloudCornerLast->empty() || laserCloudSurfLast->empty()) return;
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void visualizeGlobalMapThread() {
    ...
    // *cloudKeyPoses3D); pcl::io::savePCDFileASCII(savePCDDirectory +
    // "transformations.pcd", *cloudKeyPoses6D);
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalCornerCloud(new
    // pcl::PointCloud<pcl::PointXYZRGB>());
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalCornerCloudDS(new
    // pcl::PointCloud<pcl::PointXYZRGB>());
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalSurfCloud(new
    // pcl::PointCloud<pcl::PointXYZRGB>());
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalSurfCloudDS(new
    // pcl::PointCloud<pcl::PointXYZRGB>());
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalMapCloud(new
    // pcl::PointCloud<pcl::PointXYZRGB>()); for (int i = 0; i <
    // (int)cloudKeyPoses3D->size(); i++) {
    //     *globalCornerCloud += *transformPointCloud(cornerCloudKeyFrames[i],
    //     &cloudKeyPoses6D->points[i]); *globalSurfCloud   +=
    //     *transformPointCloud(surfCloudKeyFrames[i],
    //     &cloudKeyPoses6D->points[i]); cout << "\r" << std::flush <<
    //     "Processing feature cloud " << i << " of " << cloudKeyPoses6D->size()
    //     << " ...";
    // }
    // downSizeFilterCornerRGB.setInputCloud(globalCornerCloud);
    // downSizeFilterCornerRGB.filter(*globalCornerCloudDS);
    ...
    // *globalSurfCloudDS); *globalMapCloud += *globalCornerCloud;
    // *globalMapCloud += *globalSurfCloud;
  ...
  void publishGlobalMap() {
    ...
    pcl::PointCloud<PointType>::Ptr globalMapKeyPosesDS(
        new pcl::PointCloud<PointType>());
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalMapKeyFrames(
        new pcl::PointCloud<pcl::PointXYZRGB>());
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr globalMapKeyFramesDS(
        new pcl::PointCloud<pcl::PointXYZRGB>());
  ...
  void performLoopClosure() {
    ...
    *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
    ...
    // correctionLidarFrame = correctionLidarFrame.inverse();
    ...
    // Vector6 << noisePower, noisePower,noisePower,noisePower,noisePower,noisePower;
    
    // Vector6 << noiseScore*10, noiseScore*10, noiseScore*10, noiseScore*10,
    // noiseScore*10, noiseScore*10;
  ...
  bool detectLoopClosureDistance(int *latestID, int *closestID) {
    ...
    // for (int i = (int)pointSearchIndLoop.size() - 1; i >= 0; --i) {
    //   int id = pointSearchIndLoop[i];
    //   if (abs(copy_cloudKeyPoses6D->points[id].time - timeLaserInfoCur) >
    //       historyKeyframeSearchTimeDiff) {
    //     loopKeyPre = id;
  ...
  void loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr &nearKeyframes,
    ...
    for (int i = -searchNum; i <= searchNum; ++i) {
      ...
      pcl::PointCloud<PointType>::Ptr corner_temp(
          new pcl::PointCloud<PointType>());
      pcl::PointCloud<PointType>::Ptr surf_temp(
          new pcl::PointCloud<PointType>());
      ...
      *nearKeyframes += *transformPointCloud(
          corner_temp, &copy_cloudKeyPoses6D->points[keyNear]);
      *nearKeyframes += *transformPointCloud(
          surf_temp, &copy_cloudKeyPoses6D->points[keyNear]);
    }
      ...
      if (laserCloudMapContainer.find(thisKeyInd) !=
        ...
        *laserCloudSurfFromMap += laserCloudMapContainer[thisKeyInd].second;
        ...
        pcl::PointCloud<PointType> laserCloudCornerTemp = *transformPointCloud(
            corner_temp, &cloudKeyPoses6D->points[thisKeyInd]);
        pcl::PointCloud<PointType> laserCloudSurfTemp = *transformPointCloud(
            surf_temp, &cloudKeyPoses6D->points[thisKeyInd]);
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void addOdomFactor() {
    if (cloudKeyPoses3D->points.empty()) {
      ...
      std::cout<< "Add prior factor for first pose" << std::endl;
    } else {
    //   noiseModel::Diagonal::shared_ptr odometryNoise =
    //       noiseModel::Diagonal::Variances(
    //           (Vector(6) << 1e-6, 1e-6, 1e-6, 1e-4, 1e-4, 1e-4).finished());

    std::cout<<"Add odometry factor : " <<cloudKeyPoses3D->size()<< std::endl;

   
#if 1
    double noiseScale = 1e-6;
    double noiseRotScale = 1e-4;
    // if(cloudKeyPoses3D->size() < 10)
    // {
    //     noiseScale = 0.001;
    //     std::cout<<"Set odometry noise scale to "<<noiseScale<<std::endl;
    // }

      noiseModel::Diagonal::shared_ptr odometryNoise =
          noiseModel::Diagonal::Variances(
              (Vector(6) << noiseScale, noiseScale, noiseScale, noiseRotScale, noiseRotScale, noiseRotScale).finished());
#else
      noiseModel::Diagonal::shared_ptr odometryNoise =
      noiseModel::Diagonal::Variances((Vector(6) << 1e-1, 1e-1, 1e-1, 1/2,
      1/2, 1/2).finished());
#endif
      gtsam::Pose3 poseFrom =
  ...
  void saveKeyFramesAndFactor() {
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr thisCornerKeyFrame(
        new pcl::PointCloud<pcl::PointXYZRGB>());
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr thisSurfKeyFrame(
        new pcl::PointCloud<pcl::PointXYZRGB>());
